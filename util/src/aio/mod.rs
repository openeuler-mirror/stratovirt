        ...
        match self.opcode {
            OpCode::Preadv => {
                ...
                loop {
                    ...
                    unsafe { iov_from_buf_direct(iovecs, src) }.and_then(|v| {
            ...
            OpCode::Pwritev => {
                ...
                loop {
                    ...
                    // SAFETY: iovecs is generated by address_space.
                    unsafe { iov_to_buf_direct(iovecs, 0, dst) }.and_then(|v| {
                        if v == real_nbytes as usize {
                            Ok(())
                        } else {
...
/// # Safety
///
/// Caller should has valid hva address.
pub unsafe fn mem_from_buf(buf: &[u8], hva: u64) -> Result<()> {
    let mut slice = std::slice::from_raw_parts_mut(hva as *mut u8, buf.len());
    slice
        .write_all(buf)
...
pub unsafe fn iov_from_buf_direct(iovec: &[Iovec], buf: &[u8]) -> Result<usize> {
    let mut start: usize = 0;
    let mut end: usize = 0;

    for iov in iovec.iter() {
        end = cmp::min(start + iov.iov_len as usize, buf.len());
        // iov len is not less than buf's.
...
///
/// Caller should has valid hva address.
pub unsafe fn mem_to_buf(mut buf: &mut [u8], hva: u64) -> Result<()> {
    let slice = std::slice::from_raw_parts(hva as *const u8, buf.len());
    buf.write_all(slice)
...
///
/// Caller should has valid iovec.
pub unsafe fn iov_to_buf_direct(iovec: &[Iovec], offset: u64, buf: &mut [u8]) -> Result<usize> {
...
///
/// Caller should has valid iovec.
pub unsafe fn iovec_write_zero(iovec: &[Iovec]) {
    for iov in iovec.iter() {
...
mod tests {
    ...
    fn test_iovec_write_zero() {
        ...
        // SAFETY: iovecs has valid hva address.
