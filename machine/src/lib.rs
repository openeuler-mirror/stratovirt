...
fn handle_destroy_request(vm: &Arc<RwLock<dyn MachineOps>>) -> bool {
...
pub fn vm_run(
    vm: &Arc<RwLock<dyn MachineOps + Send + Sync>>,
    cmd_args: &arg_parser::ArgMatches,
) -> Result<()> {
    let migrate = vm.read().unwrap().get_migrate_info();
...
fn start_incoming_migration(vm: &Arc<RwLock<dyn MachineOps + Send + Sync>>) -> Result<()> {
    let (mode, path) = vm.read().unwrap().get_migrate_info();
    match mode {
        MigrateMode::File => {
