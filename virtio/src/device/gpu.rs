...
impl Gpu {
    ...
    fn snapshot_resource(blob_len: &mut u64, res: &GpuResource) -> GpuResourceState {
        let image = Self::pixman_data(res).map(|data| Self::blob_range(blob_len, data.len()));
    ...
    fn append_resource_blob(data: &mut Vec<u8>, res: &GpuResource) {
        if let Some(image) = Self::pixman_data(res) {
    ...
    fn build_state_blob(&self) -> Result<Vec<u8>> {
        ...
        for res in &rt.resources_list {
            Self::append_resource_blob(&mut data, res);
        }
    ...
    fn rebuild_resource(
        ...
        if let Some(range) = &state.image {
            ...
            let image_len = (get_image_stride(res.pixman_image) as usize)
                .checked_mul(get_image_height(res.pixman_image) as usize)
