...
"""MMDS end-to-end tests for MicroVM (LightMachine)."""
...
METADATA = {
    "instanceID": "inst-micro-001",
    "envID": "env-micro-abc",
    "address": "10.0.0.42",
}

# Matches ANSI escape sequences (with or without leading ESC) and VT100
# bracketed-paste mode sequences that bash injects around command output.
# The debug log shows them WITHOUT the ESC byte (aexpect strips it), so we
# cover both forms: '\x1b[...' and bare '[?NNNNl/h'.
_VT100_RE = re.compile(r'\x1b\[[0-9;?]*[A-Za-z]|\[\?[0-9;]*[lh]')


def _clean(s):
    """Strip VT100/ANSI codes from a serial output string."""
...
def _read_file(vm, path):
    """Read a file from the guest and return its content, stripped of
    VT100 escape codes and shell prompt noise.

    The trailing '&& echo' appends a newline so that the shell prompt appears
    on its own line rather than immediately after the file content; without it
    aexpect's cmd_status_output suffers a race where the JSON and the prompt
    arrive on the same terminal line.
    """
    _, raw = vm.serial_cmd("cat %s 2>/dev/null && echo" % path)
    cleaned = _clean(raw)
    # Drop lines that look like a shell prompt
...
def _put_mmds_config(vm, iface_id, version="V1", ipv4_address=None):
    """Send put-mmds-config QMP command; assert no error.

    qmp_command() converts underscore kwargs to hyphens, so
    network_interfaces -> "network-interfaces" which matches the
    #[serde(rename = "network-interfaces")] in MmdsConfigArgs.
    """
...
def _put_mmds_data(vm):
    """Replace MMDS data store with test metadata via QMP."""
...
def _add_mmds_route(vm):
    """Add a host route to the MMDS IP inside the guest."""
...
def _curl_get(vm, token=None, max_time=5):
    """GET MMDS endpoint inside the guest.  Returns (exit_status, body).

    Redirects curl output to a temp file so that the VT100/bracketed-paste
    escape codes that bash injects around command output never pollute the
    captured body text.
    """
    header = ""
...
def _curl_get_http_code(vm, token=None, max_time=5):
    """Return the HTTP status code string from a GET (no fail-on-error)."""
    header = ""
    if token is not None:
        header = "-H 'X-metadata-token: %s' " % token
    vm.serial_cmd(
...
def _curl_put_token(vm, ttl=21600, max_time=5):
    """PUT /latest/api/token; assert success; return token string."""
    vm.serial_cmd(
...
def test_mmds_microvm_basic_metadata_v1(microvm):
    """
    Integration: MicroVM with MMDS (V1) returns expected metadata.

    1. Launch microvm
    2. Configure MMDS V1 on the virtio-net interface via QMP
    ...
    iface_id = test_vm.taps[0]["name"]
    _put_mmds_config(test_vm, iface_id, version="V1")
    ...
    logging.info("MMDS V1 metadata validated: %s", data)


@pytest.mark.acceptance
def test_mmds_microvm_imdsv2(microvm):
    """
    IMDSv2 flow on MicroVM:

    1. Configure MMDS V2 on the virtio-net interface
    ...
    iface_id = test_vm.taps[0]["name"]
    _put_mmds_config(test_vm, iface_id, version="V2")
    _put_mmds_data(test_vm)
    _add_mmds_route(test_vm)
...
@pytest.mark.acceptance
def test_mmds_microvm_hotswap(microvm):
    """
    Hot-swap: re-calling put-mmds-config on a running MicroVM takes effect
    ...
    data = json.loads(body)
    assert data["instanceID"] == METADATA["instanceID"]
    logging.info("Hot-swap re-enable validated: %s", data)
