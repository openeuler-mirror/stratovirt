...
def _check_vsock_enable(microvm):
    ...
    if status != 0:
        status1, _ = microvm.serial_cmd("modprobe vhost_vsock")
        if status1 != 0:
...
def _get_recv_data_from_guest(microvm):
