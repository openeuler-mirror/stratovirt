    ...
    let parser = ArgParser::new("StratoVirt")
        ...
        .arg(
            ...
            .help("\n\t\tadd virtio mmio block: -device virtio-blk-device,id=<blk_id>,drive=<drive_id>[,iothread=<iothread1>][,serial=<serial_num>]; \
                   ...
                   \n\t\tadd virtio mmio pmem: -device virtio-pmem-device,id=<viomem_id>,memdev=<objmem0>[,memaddr=<68719476736>]; \
                   \n\t\tadd virtio pci pmem: -device virtio-pmem-pci,id=<viomem_id>,bus=<pcie.0>,addr=<0x4>,memdev=<objmem0>[,memaddr=<68719476736>][,multifunction=on|off]; \
