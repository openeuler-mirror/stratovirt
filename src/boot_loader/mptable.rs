...
use crate::helper::checksum::obj_checksum;
...
pub fn setup_isa_mptable(
    ...
    if u32::from(num_cpus) > MPTABLE_MAX_CPUS {
        panic!("invalid num cpus!");
    }

    let ioapic_id = num_cpus + 1;
    let header = start_addr + size_of::<FloatingPointer>() as u64;
    sys_mem
        .write_object(&FloatingPointer::new(header as u32), start_addr)
        .map_err(|_| std::io::ErrorKind::InvalidData)?;

    let mut offset = header + size_of::<ConfigTableHeader>() as u64;
    let mut sum = 0u8;
